 ...
 * `reportsToolCalls` is true, since Claude Code's NDJSON transcript reports
 * the CLI's real tool calls. Session persistence is disabled
 ...
 * same `dir`; evals of the same skill are automatically serialized by the
 * runner (see `sharedInstallDir` on `capabilities`) — evals of *different*
 * skills still race on unrelated shared state in `dir` (e.g. concurrent git
